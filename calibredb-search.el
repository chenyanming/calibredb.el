...
(defun calibredb-toggle-view-at-point ()
  ...
  (interactive)
  (cond ((get-text-property (point) 'calibredb-entry nil)
         (setq calibredb-detial-view t))
        ((get-text-property (point) 'calibredb-detail nil)
         (setq calibredb-detial-view nil)))
  (let ((inhibit-read-only t))
    (cond ((and calibredb-detial-view (get-text-property (point) 'calibredb-entry nil))
           (let* ((original (get-text-property (point) 'calibredb-entry nil))
                  (entry (cadr original))
                  (format (list (calibredb-format-item entry)))
                  (image (get-text-property (point) 'calibredb-entry nil)))
             (delete-region (line-beginning-position) (line-end-position))
             (save-excursion
               (unless (equal format "")
                 (let ((content (car format)) beg end)
                   (setq beg (point))
                   (insert content)
                   (calibredb-detail-view-insert-image image)
                   (setq end (point))
                   (put-text-property beg end 'calibredb-detail (cons format (list entry))))))))
          ((and (not calibredb-detial-view) (get-text-property (point) 'calibredb-detail nil))
           (let* ((original (get-text-property (point) 'calibredb-detail nil))
                  (entry (cadr original))
                  (format (list (calibredb-format-item entry)))
                  (id (calibredb-get-init "id" (cdr (get-text-property (point) 'calibredb-detail nil))))
                  d-beg d-end)
             ...
             (save-excursion
               (unless (equal format "")
                 (let ((content (car format)) beg end)
                   ...
                   (insert content)
                   (setq end (point))
                   (put-text-property beg end 'calibredb-entry (cons format (list entry)))))))))))
