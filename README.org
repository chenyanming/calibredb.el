...
** Column width
You can configure the column width via
...
** How to add ebooks?
1. =M-x calibredb-add=, select and mark ebooks (only ivy supported currently -
   with =ivy-hydra=, in ivy-window, press =m= to mark, other engines can only select
   one item) to add to currently library.
