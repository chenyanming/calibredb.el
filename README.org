...
Make sure the libraries are valid calibre library, where should contains a
*metadata.db* file into. Create libraries using calibre GUI or with ~M-x
calibredb-clone~ to clone the existing library to a new library.
...
2. Keywords are separated by "Spaces" (AND operation, mainly used to narrow down
   the results, the more spaces you insert, the less results.)
...
- Adjust keybindings:
    ...
    (define-key map "r" #'calibredb-search-clear-filter)
    (define-key map "R" #'calibredb-search-refresh-and-clear-filter) ; previous r
    #+END_SRC
