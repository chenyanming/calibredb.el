...
1. Tested with a library that has 8668 books, using 4.2 GHz Intel Core i7 in
   macOS 10.13.6, around =2= seconds to query the database.
...
3. The first time to enter =calibredb=, via ivy/helm/dashboard/list, is to query
   and decode the whole database. After that, all data will be saved in
...
4. Update metadata, live filter, refresh dashboard and switch library will
