...
* Q&A
The keybindings mentioned below are referred from [[keybindings]].

** What is my current library?
=M-x calibredb=, your current library shows in the header in buffer =*calibredb-search*=
Besides, variable =calibredb-root-dir= also save the current library location.
...
=M-x calibredb-add-dir= and select one directory to add, every supported formats will be added into current library.
...
2. =M-x= =calibredb-find-helm= or =calibredb-find-counsel=
   ...
   - Perform action according as shown in menu.
...
Then, in =*calibredb-search*= buffer,
 - Press =l= to open the library list and select the library you want to switch to.
 - Press =n= or =p= to switch to next or previous library.

If you are not in =*calibredb-search*= buffer, you can call =calibredb-library-list=
directly and select the library to jump.

If the library is not defined in =calibredb-library-alist=, you can call
=calibredb-switch-library= to select the path of the library that you want to
switch temporary.

** How to set metadata on ebooks?
- If you are in =*calibredb-search*= buffer, =m/u= to mark/unmark items, and =s= to
  set metadata in bulk. If you do not mark any items, actions will be performed
  to the item under cursor.
- If you are in =*calibredb-entry*= buffer, press =s= to set metadata.

** Can not remember so many keybindings?
You can preform every actions starting from ~?~.

** More examples?
Please check [[keybindings]]. You can play with every keybindings to perform
different actions. All are self documented.
