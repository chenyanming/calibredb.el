...
PS: Keybindings might be changed after the update.
...
** How to add ebooks?
1. =M-x calibredb-add=, select and mark ebooks (only ivy supported currently - with
=ivy-hydra=, in ivy-window, press =m= to mark, other engines can only select one
item) to add to currently library.

2. =M-x calibredb-add-dir= and select one
directory to add, all supported formats will be added into current library.
