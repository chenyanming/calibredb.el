...
- Add ~calibredb-yank-dispatch~, you can copy the marked items with pressing ~yy~ as org links now.
- Add "Fetch and set metadata by identifier", with keys ~sd~: ~calibredb-fetch-and-set-metadata-by-id~. Thanks [[https://github.com/tpeacock19][tpeacock19]]!
